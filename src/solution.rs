...
mod tests {
    ...
    fn sample_puzzle() -> anyhow::Result<Clash> {
        ...
        let clashes_dir = ProjectDirs::from("", "CoCtus", "coctus").expect("Unable to find project directory").data_dir().join("clashes");
    ...
    fn test_passing_solution() {
        ...
        assert!(lazy_run(clash.testcases(), &mut run_cmd, &timeout).into_iter().all(|test_run| test_run.is_successful()))
    }
